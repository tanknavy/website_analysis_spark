...
public class MockData {
	...
	//public static void mock(JavaSparkContext sc, SparkSession sparkSession){ // 2.0版使用SparkSession
		...
		for(int i=0;i<1000;i++){ //每个用户
			...
			for(int j=0;j<10;j++){ // 每个用户的连接和会话
				...
				for(int k=0;k<random.nextInt(100);k++){ //某个个用户的某个会话的某个行为
					...
					String action = actions[random.nextInt(actions.length)]; //nextInt(10)只会0-9，不会出现10
		...
		rows.clear(); //Removes all of the elements from this list 
		...
		for(int i = 0; i < 100; i ++) {
			...
			String professional = "professional" + random.nextInt(20);
			String city = "city" + random.nextInt(20);
		...
		StructType schema2 = DataTypes.createStructType(Arrays.asList( //Returns a fixed-size list backed by the specified array
				DataTypes.createStructField("user_id", DataTypes.LongType, true),
		...
		//df2.createGlobalTempView("user_visit_action"); //
		...
		//df3.createGlobalTempView("user_visit_action"); //
